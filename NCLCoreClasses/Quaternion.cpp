...
Quaternion Quaternion::LookRotation(const Vector3& forward, const Vector3& up) {
	Vector3 forwardNormalized = forward.Normalised();
	Vector3 right = Vector3::Cross(up, forwardNormalized).Normalised();
	Vector3 upAdjusted = Vector3::Cross(forwardNormalized, right);

	float m00 = right.x;
	float m01 = right.y;
	float m02 = right.z;
	float m10 = upAdjusted.x;
	float m11 = upAdjusted.y;
	float m12 = upAdjusted.z;
	float m20 = -forwardNormalized.x;
	float m21 = -forwardNormalized.y;
	float m22 = -forwardNormalized.z;
