...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath) {
	...
	//int fromX = ((int)(from.x - offset.x + nodeSize / 2) / nodeSize);
	//int fromZ = ((int)(from.z - offset.z + nodeSize / 2) / nodeSize);

	//int toX = ((int)(to.x - offset.x + nodeSize / 2 ) / nodeSize);
	//int toZ = ((int)(to.z - offset.z + nodeSize / 2 ) / nodeSize);
...
/*
	Identify a position is inside the wall or not
*/
bool NavigationGrid::IsWall(Vector3 position) {
	int X = ((int)position.x / nodeSize);
	int Z = ((int)position.z / nodeSize);

	if ((Z * gridWidth) + X > gridWidth * gridHeight) {
		return true; //outside the map
	}

	GridNode* node = &allNodes[(Z * gridWidth) + X];

	return (node->type == 'x' ? true : false);
}