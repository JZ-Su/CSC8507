...
/*
	Return a list of GameObjects that being collided with the given GameObject
*/
std::vector<GameObject*> PhysicsSystem::GetCollisionDetectionList(GameObject* o) {
	...
	for (auto i = first; i != last; i++) {
		if ((*i)->GetPhysicsObject() == nullptr || (*i) == o) {
			continue;
		}
		CollisionDetection::CollisionInfo info;
		if (CollisionDetection::ObjectIntersection(*i, o, info)) {
			result.push_back(*i);
		}
	}
	return result;
...
void PhysicsSystem::BasicCollisionDetection() {
	...
	for (auto i = first; i != last; i++) {
		if ((*i)->GetPhysicsObject() == nullptr || (*i)->GetCollisionResponse() == false) {
			continue;
		}
...
void PhysicsSystem::HandleFireballBulletCollisionLogic(GameObject* i, GameObject* j) {
	...
	if (j->GetTag() == "fireballbullet" && i->GetTag() == "item") {
		skipImpulseResolveCollision = true;
	}
...
void PhysicsSystem::HandleRollingRockCollisionLogic(GameObject* i, GameObject* j) {
	if (i->GetTag() == "rollingrock" && j->GetTag() == "item") {
		skipImpulseResolveCollision = true;
	}
	if (j->GetTag() == "rollingrock" && i->GetTag() == "item") {
		skipImpulseResolveCollision = true;
	}
...
void PhysicsSystem::HandleItemCollisionLogic(GameObject* i, GameObject* j) {
	if (i->GetTag() == "item" && j->GetTag() == "player") {
		skipImpulseResolveCollision = true;
	}
